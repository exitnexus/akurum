...
module Akurum
  class Table
    #Table::ID is used internally to organize ids that are being queried for
  ...
  #    TableClass = Table
      ...
      def initialize(id, index_sym=:PRIMARY, selection=nil)
        @id = id
        @index = self.class::TableClass.indexes[index_sym]
        @index_name = index_sym

        if (selection.kind_of? Table::Selection)
      ...
      def to_s
        "#{self.class::TableClass}_#{self.index_name}-#{self.id.join('/')}"
      end
      ...
      def split?()
        ...
        self.id.each_with_index{|contents, i|
          if(self.class::TableClass.split_column?(self.index[i]))
            key_split = true;
          end
        };
      ...
      def condition(force_no_split = false)
        ...
        self.id.each_with_index {|contents, i|
          if (self.class::TableClass.split_column?(self.index[i]) && !force_no_split)
      ...
      def cache_key
        ...
        @cache_key = if self.primary_key?
          "#{self.class::TableClass}-#{self.id.join('/')}"
        else
          "#{self.class::TableClass}_#{self.index_name}-#{self.id.join('/')}"
        end
      ...
      def ==(other)
        return other.class.const_defined?("TableClass") && 
          self.class::TableClass == other.class::TableClass &&
      ...
      def hash
        return [self.id, self.index, self.selection, self.class::TableClass].hash
      end
    
      #returns true if the Table element has modified entries that would affect its positioning
      #within the result set of the key ie. either it moved into or out of the set matched by the key.
      def match_modified?(table_element)
        return false unless table_element.kind_of? self.class::TableClass
        modified = table_element.modified_columns.map {|col| col.to_s}
        ...
        if (!(self.index[0,self.id.length] & modified).empty?)
          return (self === table_element || self.match_original?(table_element))
      ...
      def match_original?(table_element)
        return false unless table_element.kind_of? self.class::TableClass
        self.id.each_with_index {|id, i|
          ...
          begin
            ...
            if (table_element.modified?(self.index[i]))
              return false unless (id == table_element.modified_hash[self.index[i]])
            else
              return false unless (id == table_element.send(self.index[i]))
            end
          rescue SiteError #Can happen if table_element is a selection that doesn't contain the column this index is on
            return false
          end
        }
      end
    
      #returns true if the table element would be part of the result set described by this id
      def ===(table_element)
        return false unless table_element.kind_of? self.class::TableClass
        self.id.each_with_index {|id, i|
          id = id.to_s if (id.kind_of? Symbol)
